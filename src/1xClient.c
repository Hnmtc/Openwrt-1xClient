...
#define LOCKFILE "/var/run/1xClient.pid"
...
show_usage()
{
    printf( "\n"
            ...
            "\t1xClient is a software developed individually, with NO any rela-\n"
...
program_running_check()
{
    ...
    if (exit_flag) {
        if (fl.l_type != F_UNLCK) {
            ...
            fprintf (stderr, "&&Info: NO 1xClient Running.\n");
        exit (EXIT_FAILURE);
    }
...
{   
    ...
    if ( (ins_pid = program_running_check ()) ) {
        fprintf(stderr,"@@ERROR: 1xClient Already "
                            "Running with PID %d\n", ins_pid);
        exit(EXIT_FAILURE);
    }
